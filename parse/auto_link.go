...
// 将字符串形式顶级域名转换为map形式; 参数tldsStr每行对应一个顶级域名
func getValidDomainSuffixFromStr(tldsStr string) (ret map[string]bool) {
	ret = map[string]bool{}
	split := strings.Split(tldsStr, "\n")
	var l1, l2 string
...
var (
	...
	validAutoLinkDomainSuffix1 = map[string]bool{
		...
		"work": true, "group": true, "online": true, "game": true, "design": true, "link": true, "build": true,
	...
	validAutoLinkDomainSuffix = getValidDomainSuffixFromStr(allTLDs)
)
